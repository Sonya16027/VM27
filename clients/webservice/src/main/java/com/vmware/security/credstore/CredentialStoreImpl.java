...
class CredentialStoreImpl implements CredentialStore {
	...
	private File getDefaultFilePath() {
		...
		if (File.separatorChar == '/') {
			...
			path += "/" + vmwareDirName + "/" + credstoreDirName + "/"
					+ credstoreFileName;
	...
	public synchronized char[] getPassword(String host, String username)
			throws IOException {
		CredentialStoreStorage store = new CredentialStoreStorage(cachePath,
				defaultCachePath);
	...
	public synchronized boolean addPassword(String host, String username,
		...
		return store.addEntry(host, username, CredentialStoreObfuscate
				.obfuscate(host, username, password));
	}
	 ...
	 */
	public synchronized Set<String> getUsernames(String host)
			throws IOException {
		CredentialStoreStorage store = new CredentialStoreStorage(cachePath,
				defaultCachePath);
