...
import com.vmware.vim.sms.SmServiceLocator;
...
public class SmServiceConnection {
	 ...
	 *            web service url
	 * @param cookie
	 *            VC (HTTP) session cookie
	 * @param ignoreCerts
	 *            indicates whether peer certificate should be validated
	 */
	public void connect(String urlStr, String cookie, boolean ignoreCerts)
			throws Exception {
		if (_service != null) {
			disconnect();
		}
	...
	private void addAuthHeader(String cookie) throws Exception {
		SOAPHeaderElement header = new SOAPHeaderElement("urn:sm1",
				"AuthHeader");
	...
	private String getSessionString() throws Exception {
		...
		String sessionString = (String) msgContext
				.getProperty(org.apache.axis.transport.http.HTTPConstants.HEADER_COOKIE);
		return sessionString;
	}

	private void writeData(String sessionString, String fileName)
			throws Exception {
	...
	public void loadSession(String url, String fileName, boolean ignoreCerts)
			throws Exception {
		if (_service != null) {
			disconnect();
		}
