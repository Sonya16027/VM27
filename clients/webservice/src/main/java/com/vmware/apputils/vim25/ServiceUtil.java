...
public class ServiceUtil {
	...
	public static com.vmware.apputils.vim25.ServiceUtil CreateServiceUtil() {
		return new com.vmware.apputils.vim25.ServiceUtil();
	}
	...
	public void clientConnect() throws Exception {
		try {
			if (_cb.option_is_set("ignorecert")) {
				_connection.connect(_cb.getServiceUrl(), _cb.getUsername(), _cb
						.getPassword(), true);
			} else {
				_connection.connect(_cb.getServiceUrl(), _cb.getUsername(), _cb
						.getPassword(), false);
	...
	public void clientLoadSession(String cookieString) throws Exception {
		try {
			_connection.loadSession(_cb.getServiceUrl(), cookieString);
		} catch (Exception e) {
			_cb.getUtil().logException(e);
			throw e;
		}
	...
	static String[] meTree = { "ManagedEntity", "ComputeResource",
	...
	static String[] hcTree = { "HistoryCollector", "EventHistoryCollector",
	...
	public ManagedObjectReference getDecendentMoRef(
		...
		ObjectContent[] ocary = getContentsRecursively(null, root, typeinfo,
				true);
	 ...
	 *            a root folder if available, or null for default
	 * @param type
	 *            the type of the entity - e.g. VirtualMachine
	 ...
	 *            a root folder if available, or null for default
	 * @param type
	 *            type of container refs to retrieve
	 * 
	...
	public ObjectContent[] getContentsRecursively(
		...
		PropertyFilterSpec spec = new PropertyFilterSpec(null, null,
				propspecary, new ObjectSpec[] { new ObjectSpec(null, null,
						useroot, Boolean.FALSE, selectionSpecs) });

		ObjectContent[] retoc = _connection.getService().retrieveProperties(
	...
	public ObjectContent[] getObjectProperties(
		...
		return _connection.getService().retrieveProperties(usecoll,
	...
	public String waitForTask(ManagedObjectReference taskmor) throws Exception {
		Object[] result = waitForValues(taskmor, new String[] { "info.state",
	...
	public Object[] waitForValues(ManagedObjectReference objmor,
		...
		ManagedObjectReference filterSpecRef = _connection.getService()
				.createFilter(_connection.getPropCol(), spec, true);
		...
		while (!reached) {
			...
			while (retry) {
				try {
					updateset = _connection.getService().waitForUpdates(
	...
	public SelectionSpec[] buildFullTraversal() {
		...
		TraversalSpec rpToRp = new TraversalSpec(null, null, null,
				"ResourcePool", "resourcePool", Boolean.FALSE,
		...
		TraversalSpec rpToVm = new TraversalSpec(null, null, null,
				"ResourcePool", "vm", Boolean.FALSE, new SelectionSpec[] {});
		rpToVm.setName("rpToVm");
		// Traversal through ResourcePool branch
		TraversalSpec crToRp = new TraversalSpec(null, null, null,
				"ComputeResource", "resourcePool", Boolean.FALSE,
				new SelectionSpec[] { new SelectionSpec(null, null, "rpToRp"),
						new SelectionSpec(null, null, "rpToVm") });
		crToRp.setName("crToRp");
		// Traversal through host branch
		TraversalSpec crToH = new TraversalSpec(null, null, null,
				"ComputeResource", "host", Boolean.FALSE,
				new SelectionSpec[] {});
		crToH.setName("crToH");
		// Traversal through hostFolder branch
		TraversalSpec dcToHf = new TraversalSpec(null, null, null,
				"Datacenter", "hostFolder", Boolean.FALSE,
				new SelectionSpec[] { new SelectionSpec(null, null,
						"visitFolders") });
		dcToHf.setName("dcToHf");
		// Traversal through vmFolder branch
		TraversalSpec dcToVmf = new TraversalSpec(null, null, null,
				"Datacenter", "vmFolder", Boolean.FALSE,
				new SelectionSpec[] { new SelectionSpec(null, null,
						"visitFolders") });
		dcToVmf.setName("dcToVmf");
		// Recurse through all Hosts
		TraversalSpec HToVm = new TraversalSpec(null, null, null, "HostSystem",
				"vm", Boolean.FALSE, new SelectionSpec[] { new SelectionSpec(
						null, null, "visitFolders") });
		HToVm.setName("HToVm");
		// Recurse thriugh the folders
		TraversalSpec visitFolders = new TraversalSpec(null, null, null,
				"Folder", "childEntity", Boolean.FALSE, new SelectionSpec[] {
		...
		return new SelectionSpec[] { visitFolders, dcToVmf, dcToHf, crToH,
				crToRp, rpToRp, HToVm, rpToVm };
	}
