 ...
 * <p>
 * This class also creates an instance of the Logger for outputting messages to
 * the console, and parses and validates any additional command-line options
 * (creating by using the {@link OptionSpec} class) that might be passed to it
...
public class AppUtil {
	...
	public static AppUtil initialize(String name, OptionSpec[] userOptions,
	...
	public void setup() {
		...
		_connection = com.vmware.apputils.vim.ServiceConnection
	...
	public void addOptions(OptionSpec[] userOptions) throws Exception {
		for (OptionSpec userOption : userOptions) {
			if (userOption.getOptionName() != null
					&& userOption.getOptionName().length() > 0
					&& userOption.getOptionDesc() != null
					&& userOption.getOptionDesc().length() > 0
					&& userOption.getOptionType() != null
					&& userOption.getOptionType().length() > 0
					&& (userOption.getOptionRequired() == 0 || userOption
							.getOptionName().length() > 1)) {
				userOpts.put(userOption.getOptionName(), userOption);
			} else {
				System.out.println("Option " + userOption.getOptionName()
						+ " definition is not valid");
				throw new ArgumentHandlingException("Option "
						+ userOption.getOptionName()
						+ " definition is not valid");
			}
		}
	}

	public void setOptions(OptionSpec[] userOptions) throws Exception {
		userOpts.clear();
		for (OptionSpec userOption : userOptions) {
			if (userOption.getOptionName() != null
					&& userOption.getOptionName().length() > 0
	...
	/****************************************************************
	 * Creates common built-in options used (or available to be used) by all
	 * sample applications.
	 * 
	 * @param url
	 *            Required. Complete URL for the VI API Web service for the
	 *            connection.
	 * @param username
	 ...
	 * @param config
	 *            Location of an optional configuration file that contains the
	 *            required parameters.
	 * @param protocol
	 *            Protocol for the server connection (HTTP, HTTPS; HTTPS is the
	 *            default).
	 * @param server
	 *            Fully-qualified name of the server to connect to, such as
	 *            myserver.mycompany.com.
	 * @param portnumber
	 *            Server port for the VI API Web service. Defaults is 443 (for
	 *            HTTPS).
	 * @param servicepath
	 *            Service path for the VI API, such as /sdk, /sdk/webService
	 * @param sessionfile
	 *            = File containing a previously saved session cookie, for
	 *            re-establishing connection to the server.
	 * @param help
	 *            Display usage information for the application or script.
	 * @param ignorecert
	 *            Avoid the SSL server-certificate validation process.
	 */

	private void builtinOptions() {
		OptionSpec url = new OptionSpec(
				"url",
				"String",
				1,
				"Required. Complete URL for the VI API Web service to connect to",
				null);
		OptionSpec userName = new OptionSpec(
				"userName",
				"String",
				1,
				"Required. User account with privileges to connect to the host",
				null);
		OptionSpec password = new OptionSpec("password", "String", 1,
				"Required. Password for the user account", null);
		OptionSpec config = new OptionSpec("config", "String", 0,
				"Location of the VI Perl Toolkit configuration file", null);
		OptionSpec protocol = new OptionSpec("protocol", "String", 0,
				"Protocol for the server connection (HTTP, HTTPS)", null);
		OptionSpec server = new OptionSpec("server", "String", 0,
				"Server to connect to", null);
		OptionSpec portNumber = new OptionSpec("portNumber", "String", 0,
				"Port to connect to server", "443");
		OptionSpec servicePath = new OptionSpec("servicePath", "String", 0,
				"Service path used to connect to server", null);
		OptionSpec sessionFile = new OptionSpec("sessionFile", "String", 0,
				"File containing previously saved session cookie", null);
		OptionSpec help = new OptionSpec("help", "String", 0,
				"Display usage information for the application or script", null);
		OptionSpec ignorecert = new OptionSpec("ignorecert", "String", 0,
				"Ignore the server certificate validation", null);
	...
	public void parseInput(String args[]) throws Exception {
		...
		while (It.hasNext()) {
			...
			if (result == false && valid == false) {
				...
				displayUsage();
				throw new ArgumentHandlingException("Invalid Input Option '"
						+ keyValue + "'");
			}
			result = checkDatatypes(builtInOpts, keyValue, keyOptions);
			valid = checkDatatypes(userOpts, keyValue, keyOptions);
			if (result == false && valid == false) {
				System.out.println("Invalid datatype for Input Option '"
						+ keyValue + "'");
				displayUsage();
				throw new ArgumentHandlingException("Invalid Input Option '"
						+ keyValue + "'");
	...
	private void getCmdArguments(String args[]) throws Exception {
		...
		while (i < args.length) {
			...
			String opt = args[i];
			if (opt.startsWith("--")
					&& optsEntered.containsKey(opt.substring(2))) {
				System.out.println("key '" + opt.substring(2)
						+ "' already exists ");
				displayUsage();
	...
	private boolean checkDatatypes(HashMap Opts, String keyValue,
			String keyOptions) {
		boolean valid = false;
		...
		try {
			if (dataType.equalsIgnoreCase("Boolean")) {
				if (keyValue.equalsIgnoreCase("true")
						|| keyValue.equalsIgnoreCase("false")) {
					return true;
				} else {
					return false;
				}
	...
	public void validate(Object className, String functionName)
		...
		for (int i = 0; i < vec.size(); i++) {
			if (optsEntered.get(vec.get(i)) == null) {
				...
				if (missingArg.equalsIgnoreCase("password")) {
					...
					System.out.print("----ERROR: " + vec.get(i)
							+ " not specified \n");
					displayUsage();
					throw new ArgumentHandlingException("----ERROR: "
							+ vec.get(i) + " not specified \n");
				}
			}
		}
		...
		if ((optsEntered.get("sessionfile") == null)
				&& ((optsEntered.get("username") == null) && (optsEntered
						.get("password") == null))) {
			System.out
					.println("Must have one of command options 'sessionfile' or a 'username' and 'password' pair\n");
			displayUsage();
			throw new ArgumentHandlingException(
					"Must have one of command options 'sessionfile' or a 'username' and 'password' pair\n");
		}
	}

	/*
	 * Search Order for Username and Password 1. Command Line Arguments 2.
	 * Session File 3. Enviroment Variable 4. Credential Store 5. Prompt the
	 * User for Username and Password
	 */
	private void searchForUsernameAndPassword(boolean flagUsername,
			boolean flagPassword) throws Exception {
		...
		if (flagUsername && flagPassword) {
			...
			/*
			 * if(flagUsername == false) { username =
			 * System.getenv("VI_Username"); if(username == null) { username =
			 * readUsername("Enter Username: "); optsEntered.put("username",
			 * username); } }
			 */
			if (flagPassword == false) {
				...
				if (password == null) {
					...
					try {
						CredentialStore csObj = CredentialStoreFactory
								.getCredentialStore();
						if (flagUsername == false) {
							...
							if (username == null) {
								...
								if (userNameSet.size() == 1) {
									username = (String) userNameSet.iterator()
											.next();
									optsEntered.put("username", username);
								}
							}
	...
	/*
	 * taking out value of a particular key in the hashmapi.e checking for
	 * required =1 options
	 */
	...
	private void print_options(HashMap Opts) {
		...
		while (It.hasNext()) {
			String keyValue = It.next().toString();
			OptionSpec oSpec = (OptionSpec) Opts.get(keyValue);
			if ((oSpec.getOptionType() != null)
					&& (oSpec.getOptionDefault() != null)) {
			...
			if ((oSpec.getOptionDefault() != null)
					&& (oSpec.getOptionType() == null)) {
				defaultVal = oSpec.getOptionDefault();
				System.out.println("   --" + keyValue + " < default "
						+ defaultVal + " >");
			} else if ((oSpec.getOptionType() != null)
					&& (oSpec.getOptionDefault() == null)) {
				type = oSpec.getOptionType();
				System.out.println("   --" + keyValue + " < type " + type
						+ " >");
	...
	public String getOption(String key) {
		if (optsEntered.get(key) != null) {
			...
			if (((OptionSpec) builtInOpts.get(key)).getOptionDefault() != null) {
				String str = ((OptionSpec) builtInOpts.get(key))
						.getOptionDefault();
				return str;
			} else {
				return null;
			...
			if (((OptionSpec) userOpts.get(key)).getOptionDefault() != null) {
				String str = ((OptionSpec) userOpts.get(key))
						.getOptionDefault();
				return str;
			} else {
				return null;
	...
	public void loadSession() throws Exception {
		initConnection();
		getServiceUtil().clientLoadSession(getServiceUrl(),
				getOption("sessionfile"));
	}
	...
	public String getPassword() throws Exception {
		/*
		 * if (_args.length > ARG_PASSWD) { return _args[ARG_PASSWD]; } else {
		 * return ""; }
		 */
