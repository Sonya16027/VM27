...
import com.vmware.vim.VirtualFloppyDeviceBackingInfo;
...
public class VMUtils {
	...
	public ArrayList getVMs(String entity, String datacenter, String folder,
			String pool, String vmname, String host, String[][] filter)
			throws Exception {
		...
		if (datacenter != null) {
			dsMOR = cb.getServiceUtil().getDecendentMoRef(null, "Datacenter",
					datacenter);
			if (dsMOR == null) {
		...
		if (folder != null) {
			folderMOR = cb.getServiceUtil().getDecendentMoRef(tempMOR,
					"Folder", folder);
			if (folderMOR == null) {
				folderMOR2 = cb.getServiceUtil().getDecendentMoRef(null,
						"Folder", folder);
				if (folderMOR2 == null) {
					...
					if (datacenter != null) {
						dcMORChk = cb.getServiceUtil().getDecendentMoRef(
								folderMOR2, "Datacenter", datacenter);
						if (dcMORChk != null) {
							tempMOR = folderMOR2;
						} else {
							dcMORChk = cb.getServiceUtil().getDecendentMoRef(
									null, "Datacenter", datacenter);
							if (dcMORChk != null) {
								System.out
										.println("Datacenter is not under the folder "
												+ folder);
								return null;
							}
						}
		...
		if (pool != null) {
			poolMOR = cb.getServiceUtil().getDecendentMoRef(tempMOR,
					"ResourcePool", pool);
			if (poolMOR == null) {
		...
		if (host != null) {
			hostMOR = cb.getServiceUtil().getDecendentMoRef(tempMOR,
					"HostSystem", host);
			if (hostMOR == null) {
		...
		vmList = cb.getServiceUtil().getDecendentMoRefs(tempMOR,
	 ...
	 * 
	 * @param dcmor
	 *            MoRef to the Datacenter
	 * @return MoRef to a Folder returned by the hostFolder property or null if
	 *         dcmor is NOT a MoRef to a Datacenter or if the hostFolder doesn't
	 *         exist
	 * @throws Exception
	 */
	 ...
	 * @throws Exception
	 */
	public ManagedObjectReference getHost(ManagedObjectReference hostFolderMor,
			String hostname) throws Exception {
		ManagedObjectReference hostmor = null;

		if (hostname != null) {
			hostmor = cb.getServiceUtil().getDecendentMoRef(hostFolderMor,
					"HostSystem", hostname);
		} else {
			hostmor = cb.getServiceUtil().getFirstDecendentMoRef(hostFolderMor,
					"HostSystem");
		}
...
//							}
	...
	public void browseArrayList(ArrayList arrList) {
		try {
			...
			while (iterator.hasNext()) {
				ObjectContent[] ocary = cb.getServiceUtil()
	...
	public VirtualMachineConfigSpec createVmConfigSpec(String vmName,
		...
		ConfigTarget configTarget = getConfigTargetForHost(computeResMor,
				hostMor);
		VirtualDevice[] defaultDevices = getDefaultDevices(computeResMor,
				hostMor);
	 ...
	 */
	public ConfigTarget getConfigTargetForHost(
			ManagedObjectReference computeResMor, ManagedObjectReference hostMor)
			throws Exception {
		ManagedObjectReference envBrowseMor = cb.getServiceUtil().getMoRefProp(
