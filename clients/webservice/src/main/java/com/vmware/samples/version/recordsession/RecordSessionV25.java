       ...
       try {          
          ...
          if(flag) {
             if(ecb.get_option("snapshotname") == null || 
                ...
                System.out.println("snapshotname and description arguments are " + 
             ...
             ManagedObjectReference reconfigTaskMOR = 
                  ecb.getServiceConnection3().getService().reconfigVM_Task(vmmor1,
                                                                 configSpec);
              String reconfigResult = ecb.getServiceUtil3().waitForTask(reconfigTaskMOR);
             if(reconfigResult.equalsIgnoreCase("sucess")){ 
                  ManagedObjectReference recordingTaskMOR =
                   ecb.getServiceConnection3().getService().startRecording_Task(vmmor1,
                                                      ecb.get_option("snapshotname"),
                                                      ecb.get_option("description"));
                 String recordingResult = ecb.getServiceUtil3().waitForTask(recordingTaskMOR);
                 if(recordingResult.equalsIgnoreCase("sucess")){ 
                    ManagedObjectReference stopRecordingTaskMOR =
                    ecb.getServiceConnection3().getService().stopRecording_Task(vmmor1);
                    String stopResult = ecb.getServiceUtil3().waitForTask(stopRecordingTaskMOR);
             ...
             (VirtualMachineSnapshotTree[])ecb.getServiceUtil3().getDynamicProperty(
                                                     vmmor1,
                                                     "snapshot.rootSnapshotList");
             if(tree !=null && tree.length != 0) {
                ManagedObjectReference taskMor = 
                   ecb.getServiceConnection3().getService().removeAllSnapshots_Task(vmmor1);
