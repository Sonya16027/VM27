...
public class PropertyCollector {
   ...
   public static void main(String[] args) throws Exception {
      try{           
         ...
         cb = AppUtil.initialize("PropertyCollector",
         ...
         if (dcMoRef == null) {
            ...
            if (vmMoRef == null) {
               System.out.println("The virtual machine with DNS '"+ vmDnsName 
                  + "' not found ");
            }
            else{
               ...
               VirtualMachinePowerState vmState 
                  ...
                  if(vmName != null && vmState != null) {
                     System.out.println("The VM with DNS name \'"+vmDnsName
                                       +"\' is named \'"+vmName+
           ...
           if(e instanceof org.apache.axis.AxisFault) {
              ...
              for(int i=0; i<errors.length; i++) {
                 ...
                 else if(errors[i].toString().indexOf("NotSupportedFault: null") != -1) {
                    System.out.println(errors[i].toString() + " " 
   ...
   static void printObjectContent(ObjectContent[] ocs, 
      String title) {
   ...
   //
   static ObjectContent[] getDatacenters() 
   ...
   static ObjectContent[] getVMInfo(ManagedObjectReference vmMoRef)
      ...
      vmPropSpec.setPathSet(new String[] { 
              "name", 
      ...
      taskPropSpec.setPathSet(new String[] 
         { "info.name", "info.completeTime" });
   ...
   static class VMInfo {
      ...
      void setGuestToolsStatus(VirtualMachineToolsStatus 
            guestToolsStatus) {
      ...
      void setGuestHeartbeatStatus(ManagedEntityStatus 
            guestHeartbeatStatus) {
      ...
      void setRuntimePowerState(VirtualMachinePowerState 
            runtimePowerState) {
      ...
      void setSummaryQuickStatsOverallCpuUsage(int 
            summaryQuickStatsOverallCpuUsage) {
         this.summaryQuickStatsOverallCpuUsage = 
              summaryQuickStatsOverallCpuUsage;
      }
      int getSummaryQuickStatsOverallCpuUsage() {
         return summaryQuickStatsOverallCpuUsage;
      }
      void setSummaryQuickStatsHostMemoryUsage(int 
            summaryQuickStatsHostMemoryUsage) {
            this.summaryQuickStatsHostMemoryUsage = 
                summaryQuickStatsHostMemoryUsage;
      }
      int getSummaryQuickStatsHostMemoryUsage() {
         return summaryQuickStatsHostMemoryUsage;
      }
      void setSummaryQuickStatsGuestMemoryUsage(int 
            summaryQuickStatsGuestMemoryUsage) {
            this.summaryQuickStatsGuestMemoryUsage = 
                summaryQuickStatsGuestMemoryUsage;
      }
      ...
      public String toString() {
         ...
         "  Name               : ").append(getName())
         .append("\n").append("  Guest OS Name      : ")
         .append(getConfigGuestFullName()).append(
         "\n").append("  Num vCPU           : ")
         .append(getConfigHardwareNumCPU()).append(
         "\n").append("  Memory             : ")
         .append(getConfigHardwareMemoryMB()).append(
         "\n").append("  VMware Tools       : ")
         .append(getGuestToolsStatus()).append("\n")
         .append("  Guest Heartbeat    : ").append(
            getGuestHeartbeatStatus()).append(
                 "\n").append("  Guest IP Address   : ")
                        .append(getGuestIpAddress()).append("\n")
                        .append("  Guest DNS Name     : ").append(
                        getGuestHostName()).append("\n")
                       .append("  State              : ")
                       .append(getRuntimePowerState())
                       .append("\n")
                       .append("  Host               : ")
                       .append(getHostSystemName())
                       .append("\n")
                       .append("  Active Tasks       : ");
         if (getTaskInfoNames() != null) {
            for (int tini = 0; tini < getTaskInfoNames()
               .size(); ++tini) {
               sb.append(getTaskInfoNames().get(tini))
               .append(" ");
            }
         }
            sb
            .append("\n")
            .append("  CPU Usage          : ")
            .append(getSummaryQuickStatsOverallCpuUsage())
            .append(" MHz\n")
            .append("  Host Memory Usage  : ")
            .append(getSummaryQuickStatsHostMemoryUsage())
            .append(" MB\n")
            .append("  Guest Memory Usage : ")
            .append(
               getSummaryQuickStatsGuestMemoryUsage())
                  .append(" MB\n")
                  .append("  Datastores         : \n");
            if (getDatastoreInfos() != null) {
                for (int di = 0; 
                    di < getDatastoreInfos().size(); ++di) {
                    DatastoreInfo dInfo = (DatastoreInfo)
                    getDatastoreInfos().get(di);
                    sb.append("    Name       : ").append(
                            dInfo.getName()).append("\n")
                            .append("    Free Space : ")
                            .append(dInfo.getFreeSpace() 
                                    / 1024 / 1024 / 1024)
                                    .append(" GB\n");
                    int cap = 0;
            ...
            if (getNetworkNames() != null) {
                for (int nni = 0; 
                    nni < getNetworkNames().size(); ++nni) {
                    sb.append("    Name : ").append(
                            getNetworkNames().get(nni)).append(
                            "\n");
                }
            }
            return sb.toString();
                ...
                if ("VirtualMachine".equals(type)) {
                    ...
                    if (dps != null) {
                        ...
                        for (int j = 0; j < dps.length; ++j) {
                            ...
                            } else if ("config.guestFullName".equals(
                                    dp.getName())) {
                                vmInfo.setConfigGuestFullName(
                                        (String) dp.getVal());
                            } else if ("config.hardware.memoryMB"
                                    .equals(dp.getName())) {
                                vmInfo.setConfigHardwareMemoryMB(
                                        ((Integer) dp.getVal()).
                                            intValue());
                            } else if ("config.hardware.numCPU"
                                    .equals(dp.getName())) {
                                vmInfo
                                .setConfigHardwareNumCPU(((Integer)
                                        dp.getVal()).intValue());
                            } else if ("guest.toolsStatus".equals(
                                    dp.getName())) {
                                vmInfo.setGuestToolsStatus(
                                        (VirtualMachineToolsStatus) 
                                        dp.getVal());
                            } else if ("guestHeartbeatStatus".equals(
                                    dp.getName())) {
                                vmInfo.setGuestHeartbeatStatus(
                                        (ManagedEntityStatus) dp
                                            .getVal());
                            } else if ("guest.ipAddress".equals(dp
                                    .getName())) {
                                vmInfo.setGuestIpAddress((String) dp
                                        .getVal());
                            } else if ("guest.hostName".equals(dp
                                    .getName())) {
                                vmInfo.setGuestHostName((String) dp
                                        .getVal());
                            } else if ("runtime.powerState".equals(dp
                                    .getName())) {
                                vmInfo
                                .setRuntimePowerState(
                                        (VirtualMachinePowerState) dp
                                        .getVal());
                            } else if (
                                "summary.quickStats.overallCpuUsage"
                                    .equals(dp.getName())) {
                                vmInfo.
                                setSummaryQuickStatsOverallCpuUsage(
                                        ((Integer) dp
                                        .getVal()).intValue());
                            } else if (
                                "summary.quickStats.hostMemoryUsage"
                                    .equals(dp.getName())) {
                                vmInfo
                                .setSummaryQuickStatsHostMemoryUsage(
                                        ((Integer) dp.getVal()).
                                        intValue());
                            } else if (
                                "summary.quickStats.guestMemoryUsage"
                                    .equals(dp.getName())) {
                                vmInfo
                                .setSummaryQuickStatsGuestMemoryUsage(
                                        ((Integer) dp.getVal()).
                                        intValue());
                            }
                        }
                    ...
                    if (dps != null) {
                        for (int j = 0; j < dps.length; ++j) {
                            DynamicProperty dp = dps[j];
                            if ("name".equals(dp.getName())) {
                                vmInfo.setHostSystemName((String) dp
                                        .getVal());
                            }
                        }
                    }
                    ...
                    if (dps != null) {
                        ...
                        if (!taskCompleted) {
                            if (vmInfo.getTaskInfoNames() == null) {
                                vmInfo
                                .setTaskInfoNames(new ArrayList());
                            }
                    ...
                    if (dps != null) {
                        for (int j = 0; j < dps.length; ++j) {
                            ...
                            if ("info".equals(dp.getName())) {
                                if (vmInfo.getDatastoreInfos() 
                                        == null) {
                                    vmInfo.setDatastoreInfos(
                                            new ArrayList());
                                }
                                vmInfo.getDatastoreInfos().add(
                                        dp.getVal());
                            }
                        }
                    }
                    ...
                    if (dps != null) {
                        for (int j = 0; j < dps.length; ++j) {
                            DynamicProperty dp = dps[j];
                            if ("name".equals(dp.getName())) {
                                if (vmInfo.getNetworkNames() 
                                        == null) {
                                    vmInfo.setNetworkNames(
                                            new ArrayList());
                                }
                                vmInfo.getNetworkNames().add(
                                        dp.getVal());
                            }
                        }
                    }
    ...
    RemoteException {
        ...
        // retrieveProperties() returns the properties
        // selected from the PropertyFilterSpec
        return service.retrieveProperties(content
    ...
    //
    static ObjectContent[] getNetworkInfo(
        ManagedObjectReference dcMoRef)
        throws InvalidProperty, RemoteException {
        ...
        PropertySpec vmPropSpec = new PropertySpec();
        vmPropSpec.setType("VirtualMachine");
        vmPropSpec.setPathSet(new String[] { "network", "name",
        ...
        // PropertySpec for the call
        PropertyFilterSpec pfSpec = new PropertyFilterSpec();
        pfSpec.setPropSet(new PropertySpec[] { networkPropSpec,
    ...
    static void printNetworkInfo(ObjectContent[] ocs) {
        ...
        if (ocs != null) {
            for (int i = 0; i < ocs.length; ++i) {
                ...
                if ("Network".equals(type)) {
                    ...
                    // Put them in the Map
                    networksByNetwork.put(oc.getObj().get_value(),
                            network);
                  // Create our Host objects
                    ...
                    for (int n = 0; n < network.length; ++n) {
                        ...
                        if (hl == null) {
                            hl = new ArrayList();
                            hostsByNetwork.put(network[n].get_value(),
                                    hl);
                        }
                        hl.add(host);
                        hostByHost.put(
                                host.getMoRef().get_value(), host);
                    }
                  // Create our VirtualMachine objects
                } else if ("VirtualMachine".equals(type)) {
                    VirtualMachine cVm = 
                        new VirtualMachine(oc.getObj());
                    ManagedObjectReference[] network = null;
                    DynamicProperty[] dps = oc.getPropSet();
                    if (dps != null) {
                        for (int j = 0; j < dps.length; ++j) {
                    ...
                    for (int n = 0; n < network.length; ++n) {
                        VirtualMachine vm = new VirtualMachine(
                                ...
                                cVm.getQuickStats());
                        ArrayList vml = (ArrayList) vmsByNetwork
                        .get(network[n].get_value());
                        if (vml == null) {
                            vml = new ArrayList();
                            vmsByNetwork.put(network[n].get_value(),
                                    vml);
                        }
                        vml.add(vm);
                    }
        ...
        for (Iterator nit = networksByNetwork.values().iterator(); 
            nit.hasNext();) {
            Network network = (Network) nit.next();
            ArrayList vms = (ArrayList) 
            vmsByNetwork.get(network.getMoRef().get_value());
            ArrayList hosts = (ArrayList) hostsByNetwork
            .get(network.getMoRef().get_value());
            ...
            if(vms != null) {
                for (Iterator vmIt = vms.iterator(); vmIt.hasNext();) {
                    ...
                    Host host = 
                        (Host) hostByHost.get(vm.getHost().get_value());
                    
                    Integer cpuUsage = 
                        vm.getQuickStats().getOverallCpuUsage();
                    Integer memUsage = 
                        vm.getQuickStats().getHostMemoryUsage();
            ...
            if(hosts != null) {
                for (Iterator hostIt = hosts.iterator(); 
                    hostIt.hasNext();) {
                    Host host = (Host) hostIt.next();
                    
                    Integer cpuUsage = 
                        host.getQuickStats().getOverallCpuUsage();
