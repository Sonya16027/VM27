...
public class CIMUtil {
   ...
   public void init(String nsStr, String url, String userName, String password) {
      ...
      try {
         ...
         // CIMClient is deprecated, but, the WBEMClient interface doesnt exist
         client = new CIMClient(ns, user, passwordCre, CIMClient.CIM_XML);
      } catch (Exception e) {
         System.out.println("Unable to initialize CIMUtil : " + e.getMessage());
      }
   }
   ...
   public Enumeration enumerateCIMInstances(String className,
      ...
      try {
         ...
         enu = client.enumerateInstances(cop, deep, false, false, false,propertyList);
      } catch (Exception e) {
         System.out.println("Enumerating : " + className +
    ...
    *
    * wrapper for associators call with some basic error handling
    * parameters are as defined in the Wbem Javadoc.
    * 
    * @param inst
   ...
   public Enumeration associatedCIMInstances(CIMInstance cimInst,
      ...
      Enumeration enu =  associatedCIMInstances(cimInst,
                                                assocatedClassName,
                                                null,
                                                null,null,
                                                true,true,null);
    ...
    * 
    * @param inst  instance to execute operation on
   ...
   public void execOperation(CIMInstance inst, String op) {
      ...
      try {
         ...
         if (retVal != null) {
            System.out.println(op + " return value : " + retVal.getValue().toString());
         } else {
            System.out.println(op + " has no return value.");
         }
      } catch (Exception e) {
         System.out.println("InvokeMethod : " + op +
                            " : Exception : " + e.getMessage());
      }
   }
   ...
   public void printCimInstance(CIMInstance inst) {
      ...
      try {
         ...
         System.out.println("Dumping Instance : Exception : " + e.getMessage());
      }
   }
   ...
   private void printCimObjectPath(CIMObjectPath path, int prefixWhiteSpace) {
      ...
      try {
         ...
         System.out.println("Dumping ObjectPath : Exception : " + e.getMessage());
      }
   }
   ...
   private void printCimProperty(CIMProperty prop, int prefixWhiteSpace) {
      ...
      try {
         if (!prop.isKey() && ((prop.getValue() == null && !printNullValues) || printOnlyKeys)) {
            return;
         }
         printPrefixWhiteSpace(prefixWhiteSpace);
         ...
         if (prop.getValue() == null) {
            ...
            int objPathWhiteSpace =
               prefixWhiteSpace + KEYPREFIXLEN + prop.getName().length() + VALSUFFIXLEN;
            printCimObjectPath((CIMObjectPath)prop.getValue().getValue(), objPathWhiteSpace);
         ...
         System.out.println("Dumping property : Exception : " + e.getMessage());
      }
   }
