...
import com.vmware.apputils.Catalog;
import com.vmware.apputils.Log;
import com.vmware.apputils.OptionSpec;
import com.vmware.apputils.PropertyType;
...
import com.vmware.vim.sms.FilterSpec;
import com.vmware.vim.sms.FilterSpecComparisonOperator;
import com.vmware.vim.sms.FilterSpecConstraint;
import com.vmware.vim.sms.FilterSpecLogicalOperator;
import com.vmware.vim.sms.Metadata;
import com.vmware.vim.sms.QueryResult;
import com.vmware.vim.sms.QuerySpec;
import com.vmware.vim.sms.RowData;
import com.vmware.vim.sms.SmPortType;
import com.vmware.vim.sms.SortSpec;
 ...
 * 
 * b&gt;Command line:&lt;/b&gt; 
 * un.bat com.vmware.samples.sms.QueryDatastoresInDcWithFilter --url &lt;webserviceurl&gt; 
 ...
 * 
 * un.bat com.vmware.samples.sms.QueryDatastoresInDcWithFilter --url http://localhost/sdk --username administrator --password password --ignorecert 
...
public class QueryDatastoresInDcWithFilter {
	...
	public void execute() throws Exception {
		...
		sortPropertyName = cb.getOption("sortPropertyName");
		...
		QueryResult entityResult = service
				.queryList(si, null, entityType, null);
		...
		// Iterates over each instance of entityType and lists instances of
		// relEntity associated with it after applying QuerySpec.
		for (int j = 0; j < metadata.getNumRows(); ++j) {
			...
			QueryResult relEntityResult = service.queryList(si, entityRef,
					relEntityType, querySpec);

			System.out.println("For datacenter - " + (j + 1) + " Retrieved "
					+ relEntityResult.getMetadata().getNumRows()
					+ " datastore(s)");

			printFormattedQueryListResult(relEntityResult,
					relEntityTypeSpecified);
		}
	}
	...
	public FilterSpec getFilterSpec() throws Exception {
		...
		constraint1[0]
				.setComparisonOperator(FilterSpecComparisonOperator.equal);
		...
		constraint1[1]
				.setComparisonOperator(FilterSpecComparisonOperator.greaterThanOrEqual);
		...
		constraint2[0]
				.setComparisonOperator(FilterSpecComparisonOperator.beginsWith);
		...
		parentConstraint[0]
				.setChildConstraintLogicalOperator(FilterSpecLogicalOperator.or);

		parentConstraint[1] = new FilterSpecConstraint();
		parentConstraint[1].setChildConstraint(constraint2);
		parentConstraint[1]
				.setChildConstraintLogicalOperator(FilterSpecLogicalOperator.and);
	...
	public void printFormattedQueryListResult(QueryResult result,
		...
		for (int rowCounter = 0; rowCounter < numRows; rowCounter++) {
			...
			while (columnCounter < result.getMetadata().getPropertyName().length) {
				...
				// In order to do special formatting of the output depending on
				// the property type,
				// get the type of each property.

				PropertyType propertyType = null;
				if (relEntityTypeSpecified) {
					propertyType = Catalog.getPropertyType(entityType
							.getValue(), relEntityType.getValue(),
							columnNames[columnCounter]);
				} else {
					propertyType = Catalog.getPropertyType(entityType
							.getValue(), columnNames[columnCounter]);
				}
				if (propertyType.equals(PropertyType.DoublePropertyType)) {
					if (columnValues[columnCounter] == null) {
						...
						DecimalFormat decimalFormatter = new DecimalFormat(
								pattern);
						String formattedOutput = decimalFormatter.format(Double
								.parseDouble(columnValues[columnCounter]));
						System.out.println(columnNames[columnCounter] + " : "
								+ formattedOutput + " %");
					}
	...
	public static void main(String[] args) throws Exception {
		...
		cb = SmAppUtil.initialize("QueryDatastoresInDcWithFilter",
				QueryDatastoresInDcWithFilter.constructOptions(), args);
		cb.connect();
		obj.execute();
		cb.disConnect();
	}
