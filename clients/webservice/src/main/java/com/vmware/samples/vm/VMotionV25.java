...
public class VMotionV25 {
   ...
   public void useVMotion25(String [] args, String cookieString, 
                                     VMotion obj ) throws Exception {
      ...
      provisionChkr = ecb.getServiceConnection3().getServiceContent().getVmProvisioningChecker();
   ...
   /*
    *In this method we check three scenarios for Vmotion 
    *Migration Capability
    *Relocation Capability
    *Query Vmotion Compatibility
    */
   ...
   ManagedObjectReference[] dsTarget
          = (ManagedObjectReference[])ecb.getServiceUtil3().getDynamicProperty(targethostMOR, "datastore");
   ...
   if(dsMOR == null) {
    ...
    if((query) &&  (migrate) &&(relocation) ) {
      System.out.println("VMotion is feasible on VM "+vmname+" from host "+sourcehost+ " to "+targethost);
    }
    else {
     System.out.println("VMotion is not feasible on VM "+vmname+" from host "+sourcehost+ " to "+targethost);
    }
   ...
   try {
      ...
      ManagedObjectReference taskMOR = 
       ecb.getServiceConnection3().getService().checkRelocate_Task(provisionChkr,vmMOR,relSpec,null);
   ...
   private boolean checkMigrate(ManagedObjectReference vmMOR,
     ManagedObjectReference hostMOR,ManagedObjectReference poolMOR) throws Exception {
    boolean migrate = false;

   try {
      ManagedObjectReference taskMOR 
       = ecb.getServiceConnection3().getService().checkMigrate_Task(provisionChkr,vmMOR,hostMOR,poolMOR,null,null);
       String res = monitorTask(taskMOR);
      if(res.equalsIgnoreCase("sucess")) {
          migrate= true;
      }
   ...
   private boolean queryVMotionCompatibility(ManagedObjectReference vmMOR,
       ManagedObjectReference hostMOR, ManagedObjectReference targethostMOR ) throws Exception {
   ...
   try {
    ...
    String[] test = null;
    ManagedObjectReference taskMOR 
       = ecb.getServiceConnection3().getService().queryVMotionCompatibilityEx_Task(provisionChkr,vmMORs,hostMORs);
    String res = monitorTask(taskMOR);
      if(res.equalsIgnoreCase("sucess")) {
          result= true;
      }
  ...
  private ManagedObjectReference getMOR(String name, String type,ManagedObjectReference root ) throws Exception{
       ...
       = (ManagedObjectReference)ecb.getServiceUtil3().getDecendentMoRef(root, type, name);
       if(nameMOR ==null) {   
       ...
       try{
         if(dsMOR != null && dsMOR.length>0) {
            for (int i = 0; i < dsMOR.length; i++ ) {
               String dsname  = (String)ecb.getServiceUtil3().getDynamicProperty(dsMOR[i], "summary.name");
   ...
   public String monitorTask(ManagedObjectReference taskmor) throws Exception {   	  
      ...
      if (result[0].equals(TaskInfoState.success)) {      	 
         ...
         TaskInfo tinfo = (TaskInfo)ecb.getServiceUtil3().getDynamicProperty(taskmor,"info");      	 
