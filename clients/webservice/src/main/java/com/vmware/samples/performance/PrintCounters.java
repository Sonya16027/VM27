 ...
 * 
 * <p>
 * run.bat com.vmware.samples.performance.PrintCounters --url
...
public class PrintCounters {
	...
	private void printCounters() throws Exception {
		String entityType = cb.getOption("entitytype");
	...
	private void printEntityCounters(String entityType) throws Exception {
		...
		if (mor != null) {
			...
			File file = new File(cb.getOption("filename"));
			PrintWriter out = new PrintWriter(new BufferedWriter(
					new FileWriter(cb.getOption("filename"))));
			if (cInfo != null) {
			...
			System.out.println("Check " + cb.getOption("filename")
					+ " for Print Counters");
		} else {
			System.out.println(entityType + " " + cb.getOption("entityname")
					+ " not found.");
		}
	}
	...
	private ManagedObjectReference getManagedObjectReference(String entityType)
		...
		ManagedObjectReference obj = cb.getServiceUtil().getDecendentMoRef(
				null, entityType, cb.getOption("entityname"));
		return obj;
	}
