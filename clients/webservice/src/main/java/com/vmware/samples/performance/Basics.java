...
public class Basics {
	...
	private void displayBasics() throws Exception {
		...
		if (cb.getOption("info").equalsIgnoreCase("interval")) {
			getIntervals(perfMgr, service);
		} else if (cb.getOption("info").equalsIgnoreCase("counter")) {
			getCounters(perfMgr, service);
		} else if (cb.getOption("info").equalsIgnoreCase("host")) {
			ManagedObjectReference hostmor = cb.getServiceUtil()
					.getDecendentMoRef(null, "HostSystem",
							cb.getOption("hostname"));
			if (hostmor == null) {
	...
	private boolean customvalidation() throws Exception {
		boolean valid = true;
		if (cb.getOption("info").equalsIgnoreCase("host")) {
