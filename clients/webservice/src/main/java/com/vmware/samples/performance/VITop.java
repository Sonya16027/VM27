...
import com.vmware.samples.performance.widgets.StatsTable;
...
/**
* <p>This sample is an ESX-Top-like application that lets administrators specify
* the CPU and memory counters by name to obtain metrics for a specified host.
* </p>
* <p><b>Parameters:</b></p>
* <ul>
* <li>host Required. Name of the host</li>
* <li>cpu  Required. Name of an available CPU counter from which to obtain a value. For example,
* "usage," "ready," "guaranteed," and so on.</li>
* <li>memory  Required. Name of an available memory counter from which to obtain a value. For
* example, "usage," "granted," and so on.</li></ul>
*
* <p><b>Command Line:</b></p>
* <p>run.bat com.vmware.samples.performance.VITop --url [webservice url]  --username [user] 
* --password [password]  --host [FQDN_host_name] --cpu [cpu_counter_name] --memory [mem_counter_name] 
* </p>
*/
...
public class VITop {
   ...
   private void displayStats() throws Exception{
      ...
      for (int i = 0; i < statsList.length; i++) {
         ...
         midVector.add(new PerfMetricId(null, null, counterInfo.getKey(), ""));
      }

      PerfMetricId[] midList = new PerfMetricId[midVector.size()];
      for (int i = 0; i < midVector.size(); i++) {
         midList[i] = (PerfMetricId)midVector.get(i);
      }
   ...
   private void createAndShowGUI(String firstColumnName, Vector<String> statNames) {        
      try {
   ...
   private Calendar displayStats(PerfMetricId[] midList, PerfCompositeMetric compMetric)
      ...
      PerfMetricIntSeries intSeries = (PerfMetricIntSeries)entityMetric.getValue(0);
      int numSamples = entityMetric.getSampleInfo().length;

      Calendar timeStamp = entityMetric.getSampleInfo(numSamples - 1).getTimestamp();
      ...
      for (int childNum = 0; childNum < childEntityMetric.length; childNum++) {
         PerfEntityMetric childStats = (PerfEntityMetric) childEntityMetric[childNum];
         ...
         for (int i = 0; i < childStats.getValue().length; i++) {
            PerfMetricIntSeries childSeries 
               = (PerfMetricIntSeries)childStats.getValue(i);
   ...
   private void RefreshStats() {      
      try {
         PerfCompositeMetric metric 
            = cb.getConnection().getService().queryPerfComposite(perfManager, querySpec);
   ...
   private PerfCounterInfo GetCounterInfo(PerfCounterInfo[] counterInfo, 
      ...
      for (int i = 0; i < counterInfo.length; i++) {
         PerfCounterInfo info = counterInfo[i];
   ...
   private static OptionSpec[] constructOptions() {
      ...
      useroptions[0] = new OptionSpec("host","String",1
                                      ,"Name of the host"
                                      ,null);
      useroptions[1] = new OptionSpec("cpu","String",1,
                                      "Counters For CPU",
                                      null);
   ...
   public static void main(String args[]) throws Exception{
      VITop obj = new VITop();
      cb = AppUtil.initialize("VITop"
                              ,VITop.constructOptions()
                              ,args);      
      cb.connect();
      obj.displayStats();
      //cb.disConnect();
