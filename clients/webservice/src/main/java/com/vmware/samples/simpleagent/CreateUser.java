...
/*********************************************************************
 * The CreateUser class creates a user account and password stores them the local
 * credential store. CreateUser should be used in conjunction with the 
 * {@link SimpleAgent} sample application only. CreateUser and SimpleAgent 
 * demonstrate using the {@link CredentialStore} client API. 
 * CreateUser generates an arbitrary user name (Usernnnn, where nnnn is
 * a psuedo-randomly generated number) and password and stores them in the 
 * credential store on the local machine. 
 * <p> 
 * CreateUser works with ESX or ESXi only (not vCenter). 
 * It uses AppUtil and other helper classes for handling command-line arguments
 * and creating a connection for the server.</p>
 * 
 ...
 * </pre> 
 *
 *@see <a href="../../../../../ReferenceGuide/vim.AuthorizationManager.html">AuthorizationManager</a>
 *@see <a href="../../../../../ReferenceGuide/vim.host.LocalAccountManager.html">HostLocalAccountManager</a>
 */
...
public class CreateUser {
   ...
   private void createUser() throws Exception {
      ManagedObjectReference hostLocalAccountManager = 
         cb.getConnection().getServiceContent().getAccountManager();
      ...
      ManagedObjectReference rootFolder = 
         cb.getConnection().getServiceContent().getRootFolder();
      ...
      */
      ...
      cb.getConnection().getService().setEntityPermissions(hostAuthorizationManager,
                                                           rootFolder,
                                                           new Permission [] {per});
   ...
   public void customValidate() throws Exception {
    ...
    String apiType = cb.getConnection().getServiceContent().getAbout().getApiType();
      if(apiType.equalsIgnoreCase("VirtualCenter")) {
         System.out.println("CreateUser works with ESX and ESXi only.");
         throw new ArgumentHandlingException("CreateUser does not run on vCenter Server");
     }
   }     
