 ...
 * 
 * b&gt;Command:&lt;/b&gt;
 * un.bat com.vmware.samples.vm.vmpromotedisks
...
public class VMPromoteDisks {
	...
	private void promoteDeltaDisk(String[] args, String cookieString)
		...
		boolean unlink = Boolean.valueOf(ecb.getOption("unlink"));
		VirtualDisk[] vDiskArr = null;
		if (vmMOR != null) {
			if (ecb.getOption("disknames") != null) {
				String disknames = ecb.getOption("disknames");
	...
	private boolean customValidation() throws Exception {
		boolean flag = true;
		String val = cb.getOption("unlink");
