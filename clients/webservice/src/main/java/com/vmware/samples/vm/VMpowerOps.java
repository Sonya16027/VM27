 ...
 private static VMUtils vmUtils = null;
   ...
   private ArrayList getVms() throws Exception{
    ...
    // filter = new String[][] { new String[] { "summary.config.guestId", "winXPProGuest",},};
      filter =  new String[][] { new String[] { "guest.ipAddress", 
                                 ipaddress,},  
                                 new String[] { "summary.config.guestId", 
                                 guestid,}}; 
      vmList  = vmUtils.getVMs("VirtualMachine",datacenter,folder,
   ...
   private void powerOffVM(ArrayList vmList) throws Exception {
     for(int i=0; i<vmList.size(); i++) {
         ...
         try{
            ...
            taskmor = cb.getConnection().getService().powerOffVM_Task(vmMOR);
            boolean result = getTaskInfo(taskmor);
            if(result){
   ...
   private void resetVM(ArrayList vmList) throws Exception {
     for(int i=0; i<vmList.size(); i++) {
         ...
         try{
            ...
            boolean result = getTaskInfo(taskmor);
            if(result){
              System.out.println("Virtual Machine "+vmName+ " reset successfuly");
            }                
         }
         catch(Exception e){
             System.out.println("Error");
   ...
   private void suspendVM(ArrayList vmList) throws Exception {
     for(int i=0; i<vmList.size(); i++) {
         ...
         ManagedObjectReference taskmor= null;   
         try{
            System.out.println("Suspending virtualmachine '"+vmName+"'");                  
  ...
  /**
   ...
   public static void main(String[] args) throws Exception {        
      ...
      cb = AppUtil.initialize("VMpowerOps"
