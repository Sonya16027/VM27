 ...
 * un.bat com.vmware.samples.performance.History --url [webserviceurl]
...
public class History {
	...
	private void displayHistory() throws Exception {
		ManagedObjectReference hostmor = cb.getServiceUtil().getDecendentMoRef(
				null, "HostSystem", cb.getOption("hostname"));
		if (hostmor == null) {
		...
		PerfCounterInfo pci = getCounterInfo(cb.getOption("groupname"), cb
				.getOption("countername"), PerfSummaryType.average, null);
		if (pci == null) {
			System.out
		...
		int duration = Integer.parseInt(cb.getOption("duration"));
		int startTime = Integer.parseInt(cb.getOption("starttime"));
