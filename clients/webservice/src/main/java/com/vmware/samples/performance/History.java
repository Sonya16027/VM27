...
*should be displayed as per the counter and the group specified.</b>
*run.bat com.vmware.samples.performance.History --url [webserviceurl]
*--username [username]  --password [password] --hostname [name of the 
*history server]  --groupname mem --countername overhead
*</pre>
*/
   ...
   private void displayHistory() throws Exception{
      ...
      PerfInterval[] intervals 
         = (PerfInterval[])cb.getServiceUtil().getDynamicProperty(pmRef, 
                                                                 "historicalInterval");
      Integer interval = new Integer(Integer.parseInt(cb.get_option("interval")));
