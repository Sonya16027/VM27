...
public class VMSnapshot {
   ...
   private boolean revertSnapshot(ManagedObjectReference vmMor) throws Exception {
      String snapshotName = cb.get_option("snapshotname"); 
   ...
   private boolean removeAllSnapshot(ManagedObjectReference vmMor) throws Exception {
      ManagedObjectReference taskMor 
   ...
   private boolean removeSnapshot(ManagedObjectReference vmMor) throws Exception {
      String snapshotName = cb.get_option("snapshotname"); 
      ...
      if(snapmor!=null) {
         ManagedObjectReference taskMor 
            = cb.getConnection().getService().removeSnapshot_Task(snapmor,flag);      
         String res = cb.getServiceUtil().waitForTask(taskMor);
         if(res.equalsIgnoreCase("sucess")) {
            return true;
         }
   ...
   private boolean customValidation() throws Exception{
      ...
      if(op.equalsIgnoreCase("remove")) {
         if((!cb.option_is_set("snapshotname")) 
            ...
            int child = Integer.parseInt(cb.get_option("removechild"));
            if(child != 0 && child != 1) {
   ...
   public static void main(String[] args) throws Exception {     
      ...
      cb = AppUtil.initialize("VMSnapshot"
