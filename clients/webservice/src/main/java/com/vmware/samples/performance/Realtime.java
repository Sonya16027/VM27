...
* <ul>
* <li> 
* vmname   Required.  Name of the virtual machine</li></ul>
*
* <p><b>Command Line:</b></p>
* <p>run.bat com.vmware.samples.performance.Realtime --url [webserviceurl] --username [username] --password [password]
--vmname [vmname]</p>
...
public class Realtime {
      ...
      if(vmmor!=null) {
         ...
         while(true) {
            ...
            for(Iterator it = vmCpuCounters.iterator(); it.hasNext();) {
               ...
               System.out.println(++index +" - "+ pcInfo.getNameInfo().getSummary());
            }
            ...
            if(index > vmCpuCounters.size()) {
               ...
               --index;
            if(index<0) return;
               PerfCounterInfo pcInfo = (PerfCounterInfo)vmCpuCounters.get(index);
         ...
         if(aMetrics != null) {
            for(int index=0; index<aMetrics.length; ++index) {
               if(counters.containsKey(new Integer(aMetrics[index].getCounterId()))) {
   ...
   void monitorPerformance(ManagedObjectReference pmRef,
                           ...
                           ArrayList mMetrics, 
                           Map counters) throws Exception {
      PerfMetricId[] metricIds 
         = (PerfMetricId[])mMetrics.toArray(new PerfMetricId[0]);
      ...
      while(true) {       
         PerfEntityMetricBase[] pValues 
            = cb.getConnection().getService().queryPerf(pmRef,qSpecs);
         ...
         for(int vi=0; vi<vals.length; ++vi){
            PerfCounterInfo pci 
               = (PerfCounterInfo)counters.get(
                    new Integer(vals[vi].getId().getCounterId())
                  );
            if(pci != null)
   ...
   public static void main(String[] args) throws Exception {
      ...
      cb = AppUtil.initialize("RealTime"
