...
public class CIMInfo {
    ...
    * 
    * @param srcClass  source class name.
    * @param assocClass  association class name
    * @param resultClass  result class name. unused at present.
    * @param resultClassFilt  comma separated list of result classes.
    *                         if not empty, only the classes in this list are used.
    * @param srcPropFilt  property filter for source class specified
    *                     Property names and expected values are specified
    * @return AssociationFilter 
    */
   public static AssociationFilter createAssocFilter(String srcClass,
                                                     String assocClass,
                                                     String resultClass,
                                                     String resultClassFilt,
                                                     String srcPropFilt) {
   ...
   public CIMInfo(boolean printNullValues, boolean printOnlyKeys, boolean ignoreCerts) {
    ...
    * 
    * @param className  name of the CIM class
    */
    ...
    * execOperation
    * 
    * Get a single instance of the class provided, and Run the specified operation
    * for that instance of the class.
    * 
    * @param className  the class name to get an instance of
    * @param operation  the operation name to run 
    */
   public void execOperation(String className, String operation) {
      ...
      if (operation == null || operation.length() == 0) {
         System.out.println("No operation name specified for : " + className);
         return;
      }
    ...
    * 
    * print all instances of classes retrieved using association filter specified
    * 
    * @param assocFilt association filtering into
    */
   public void getAssociatedCIMInstance(CIMInfo.AssociationFilter assocFilt) {
      ...
      Enumeration srcEnum = cimUtil.enumerateCIMInstances(assocFilt.srcClassName);
      if (srcEnum == null || !srcEnum.hasMoreElements()) {
         System.out.println("No instance found for : " + assocFilt.srcClassName);
         return;
      }
      ...
      while(srcEnum.hasMoreElements()) {
         ...
         Enumeration assocEnum =
            cimUtil.associatedCIMInstances(inst, assocFilt.associatedClassName);
         ...
         while (assocEnum.hasMoreElements()) {
            ...
            if (isClassUsed(assocFilt.resultClassFilter, assocInstClass)) {
               cimUtil.printCimInstance(inst);
               System.out.println("traverses : " + assocFilt.associatedClassName + " for : ");
    ...
    * 
    * @param classFilter comma separated list of CIM leaf class names
    * @param className the class name to be verified
    ...
    * isInstanceMatch
    * 
    * If a propertyFilter is specified, checks if the CIM instance has
    * a property that matches the filter criteria specified. 
    * 
    * @param inst  the CIM instance to validate
    * @param propertyFilter the property filter specification
   ...
   private boolean isInstanceMatch(CIMInstance inst, String propertyFilter) {
      ...
      if (propertyFilter != null && propertyFilter.length() > 0) {
         ...
         StringTokenizer strTok = new StringTokenizer(propertyFilter, "!=*?&|", true);
         ...
         while (strTok.hasMoreTokens()) {
            ...
            if (matchProp != null && matchProp.getValue() != null &&
               ...
               if (matchProp.getType().getType() == CIMDataType.STRING) {
                  String propValue = (String)matchProp.getValue().getValue();
                  instMatched =
                     negateOpVal(negateOp,
                        relnOpVal(relnOp,
                                  instMatched,
                                  compareOpVal(matchValue, propValue, compareOp, false)));
               } else if (matchProp.getType().isArrayType()) {
                  // supports array values that have unique elements like states
                  String propValue = (String)matchProp.getValue().getValue().toString();
                  instMatched =
                     negateOpVal(negateOp,
                        relnOpVal(relnOp, 
                                  instMatched, 
                                  compareOpVal(matchValue, propValue, compareOp, true)));
               }
            }
         }
      } else {
   ...
   private boolean compareOpVal(String match, String prop, String op, boolean isArray) {
      if (!isArray) {
         if (op.equals("=")) {
