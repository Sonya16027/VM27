...
*
*This can be run for a single ESX 3.5 or ESX 3.5i host.
*
*<b>Parameters:</b>
*query          [required]:type of data to retrieve
*                          One of : [firmware,fansensors,frus,sensorfrus,log]
*logop           [required for "log"]:print log instance or clear the log
*                                     One of : [print,clear]
*ns              [optional] namespace for all queries to be made.
*                           defaults to "root/cimv2"
*printNulls      [optional] print properties having null or no values
*                           defaults to false
*printOnlyKeys   [optional] print only properties that are keys
*                           defaults to false
*
*<b>Command line</b>
*run.bat com.vmware.samples.ciminfo.VIWrapper --url [webserviceurl]
...
public class VIWrapper {
   ...
   private void doOperation() throws Exception {      
      ...
      if(apiType.equalsIgnoreCase("HostAgent") &&
         ...
         String nsStr = appUtil.get_option(OPT_NAMESPACE);
         if (nsStr == null || nsStr.length() == 0) {
            nsStr = OPT_VAL_CIM_NAMESPACE;
         }
         boolean printNulls =
            appUtil.get_option(OPT_PRINTNULLS).equalsIgnoreCase("true");
         boolean printKeys =
            appUtil.get_option(OPT_PRINTONLYKEYS).equalsIgnoreCase("true");
         boolean ignoreCert =
            appUtil.get_option(OPT_IGNORECERT).equalsIgnoreCase("true");

         CIMInfo cimInfo = new CIMInfo(printNulls, printKeys, ignoreCert);
         cimInfo.init(nsStr, serverUrl, appUtil.getUsername(), appUtil.getPassword());

         String query = appUtil.get_option(OPT_QUERY);
         ...
         if(query.equalsIgnoreCase(OPT_VAL_FIRMWARE)) {
            ...
            CIMInfo.AssociationFilter assocFan =
               CIMInfo.createAssocFilter("CIM_Fan",
            ...
            CIMInfo.AssociationFilter assocFru =
               CIMInfo.createAssocFilter("CIM_ComputerSystem",
                                         "CIM_SystemDevice",
                                         null,
                                         rsltClassFilt,
                                         null);
            ...
            // retrieve Sensors in a "bad" state and devices with these sensors
            // Sensor health filter is as specified below with partial
            // value matches for the Sensor.CurrentState property
            String srcPropFilt =
               "!CurrentState*Normal,Unknown,Deassert,deasserted,redundant";
            String rsltClassFilt = "OMC_Processor,OMC_PowerSupply";
            CIMInfo.AssociationFilter assocSysDev = 
               CIMInfo.createAssocFilter("CIM_Sensor",
                                         "CIM_AssociatedSensor",
                                         null,
                                         rsltClassFilt,
                                         srcPropFilt);
            ...
            // retrieve and print the System Log and then if required, clear the Log
            String logOp = appUtil.get_option(OPT_LOG_OPERATION);
   ...
   public static void main(String args []) throws Exception {
      ...
      try {
         appUtil = AppUtil.initialize("VIWrapper", VIWrapper.initOptions(), args);
         ...
         System.out.println("Exception " + e.getMessage() + " initializing. Exiting.");
      }
   }
}
