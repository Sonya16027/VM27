 ...
 * 
 * b&gt;Command:&lt;/b&gt;
 * un.bat com.vmware.samples.vm.VMRelocate
...
public class VMRelocate {
	...
	private void relocate(String[] args, String cookieString) throws Exception {
		...
		ManagedObjectReference vmMOR = getMOR(ecb.getOption("vmname"),
				"VirtualMachine", null);
		...
		while (it.hasNext()) {
			...
			if (dsArr != null) {
				for (int j = 0; j < dsArr.length; j++) {
					...
					if (name.equalsIgnoreCase(ecb.getOption("datastorename"))) {
						dsMOR = dsArr[j];
						found = true;
						j = dsArr.length + 1;
					}
		...
		if (dsMOR == null) {
			System.out.println("Datastore " + ecb.getOption("datastorename")
					+ " Not Found");
			return;
		}
		...
		if (vmMOR != null) {
			...
			String moveType = ecb.getOption("diskmovetype");
			...
			System.out.println("Virtual Machine " + ecb.getOption("vmname")
					+ " doesn't exist");
		}
	}
	...
	private boolean customValidation() throws Exception {
		boolean flag = true;
		String val = cb.getOption("diskmovetype");
