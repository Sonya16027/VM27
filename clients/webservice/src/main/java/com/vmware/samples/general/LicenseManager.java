 ...
 * un.bat com.vmware.samples.general.LicenseManager --url [webserviceurl]
...
public class LicenseManager {
	...
	private void useLicenseManager() throws Exception {
		String action = cb.getOption("action");
	...
	private void setLicenseServer() throws Exception {
		...
		if (flag) {
			...
			source.setLicenseServer(cb.getOption("serverurl"));
			try {
	...
	private void setEdition() throws Exception {
		boolean valid = validate(cb.getOption("edition"));
		if (valid) {
			try {
				cb.getConnection().getService().setLicenseEdition(licMgr, null,
						cb.getOption("edition"));
			} catch (Exception e) {
				if (e instanceof org.apache.axis.AxisFault) {
					org.apache.axis.AxisFault fault = (org.apache.axis.AxisFault) e;
					org.w3c.dom.Element[] errors = fault.getFaultDetails();
	...
	public void displayFeatureInfo() throws Exception {
		boolean valid = validate(cb.getOption("feature"));
