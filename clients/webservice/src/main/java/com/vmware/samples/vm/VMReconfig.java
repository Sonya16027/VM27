 ...
 * un.bat com.vmware.samples.vm.VMReconfig --url [webserviceurl] 
...
public class VMReconfig {
	...
	private void reConfig() throws Exception {
		String deviceType = cb.getOption("device");
	...
	private VirtualDeviceConfigSpec getCDDeviceConfigSpec() throws Exception {
		String ops = cb.getOption("operation");
		...
		if (ops.equalsIgnoreCase("Add")) {
			...
			cdDeviceBacking.setFileName("[" + dsum.getName() + "] "
					+ cb.getOption("vmname") + "/" + cb.getOption("value")
					+ ".iso");
	...
	private VirtualDeviceConfigSpec getDiskDeviceConfigSpec() throws Exception {
		...
		if (ops.equalsIgnoreCase("Add")) {
			...
			String fileName = "[" + dsName + "] " + cb.getOption("vmname")
					+ "/" + cb.getOption("value") + ".vmdk";

			diskfileBacking.setFileName(fileName);
			diskfileBacking.setDiskMode(cb.getOption("diskmode"));

			disk.setControllerKey(ckey);
			disk.setUnitNumber(unitNumber);
			disk.setBacking(diskfileBacking);
			int size = 1024 * (Integer.parseInt(cb.getOption("disksize")));
			...
			if (disk != null) {
				...
				System.out.println("No device found " + cb.getOption("value"));
				return null;
			}
		}
		return diskSpec;
	...
	private boolean customValidation() throws Exception {
		boolean flag = true;
		String device = cb.getOption("device");
		...
		if (device.equalsIgnoreCase("disk")) {
			if (Integer.parseInt(cb.getOption("disksize")) <= 0) {
				System.out.println("Disksize must be a greater than zero");
				flag = false;
			}
		}
		if (cb.option_is_set("operation")) {
			if (cb.getOption("operation").equalsIgnoreCase("add")
					|| cb.getOption("operation").equalsIgnoreCase("remove")) {
			} else {
