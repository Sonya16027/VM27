 ...
 * un.bat com.vmware.samples.httpfileaccess.GetVMFiles --url [webserviceurl] 
...
public class GetVMFiles {
	...
	private void getVM() throws Exception {
		File file = new File(cb.getOption("localpath"));
		if (!file.exists()) {
			System.out.println("Not able to find " + cb.getOption("localpath"));
			return;
		}
		ManagedObjectReference vmmor = getVmMor(cb.getOption("vmname"));
		if (vmmor != null) {
			...
			if (vmDirectory[0] != null) {
				...
				String configurationDir = vmDirectory[0].substring(
						vmDirectory[0].indexOf("]") + 2, vmDirectory[0]
								.lastIndexOf("/"));
				boolean success = new File(cb.getOption("localpath") + "/"
	...
	private void getData(String urlString, String fileName) throws Exception {
		...
		boolean flag = (new File(cb.getOption("localpath") + "/" + dir))
				.mkdir();
		fileName = cb.getOption("localpath") + "/" + dir + "/" + fName;
