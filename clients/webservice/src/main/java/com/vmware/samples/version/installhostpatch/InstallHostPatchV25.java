...
public class InstallHostPatchV25 {
   ...
   public static void queryHostComponents(
      ...
      ManagedObjectReference sicv25 
         = VersionUtil.convertManagedObjectReference(sic);
       
      ProductComponentInfo [] pcis = 
         ecb.getServiceConnection3().getService().retrieveProductComponents(
         sicv25);     
       
      if(pcis.length > 0)
         System.out.println("\nProduct Components info \n");
      for(int i=0; i<pcis.length; i++) {
         ProductComponentInfo pci = pcis[i];
   ...
   public static void installHostComponents(String [] args,String cookieString) 
      ...
      if(hmorList.get(0)!=null) {
         ...
         String fileName = ecb.get_option("bundle");
   ...
   private static void installUpdate(ManagedObjectReference hmor) throws Exception {      
      HostConfigManager cm 
         = (HostConfigManager)ecb.getServiceUtil3().getDynamicProperty(hmor,
                                                                      "configManager");
      ...
      if(pm != null) {
         ...
         ServiceContent sc = 
            (ServiceContent)ecb.getServiceUtil3().getDynamicProperty(sic,
                                                                     "content");
         ...
         String productStr = productLineId 
                             + " " + version
                             + " " + localVersion;        
         ...
         if(bundleProductStr.equalsIgnoreCase(productStr)) {
            ...
            System.out.println("$bundle_product_str does not apply to target host " 
                               + productStr);	
         }
      }
      else {
   ...
   private static void installComponents(ManagedObjectReference pm) throws Exception {
      ...
      for(int i=0; i < listOfPackage.getLength() ; i++) {
         ...
         if(packageNode.getNodeType() == Node.ELEMENT_NODE){
            ...
            Element nameElement = (Element)packageElement.getElementsByTagName(
                                  "name").item(0);
            NodeList nameList = nameElement.getChildNodes();                   
            String pName = ((Node)nameList.item(0)).getNodeValue();           
            
            NodeList scanData = packageElement.getElementsByTagName(
                                                                   "scanData");
            Element scanElement = (Element)scanData.item(0);
            
            Element scanNameElement = (Element)scanElement.getElementsByTagName(
                                      "name").item(0);
            NodeList scanNameList = scanNameElement.getChildNodes();
            String scanName = ((Node)scanNameList.item(0)).getNodeValue();           
            
            Element scanRelElement = (Element)scanElement.getElementsByTagName(
                                      "rel").item(0);
            NodeList scanRelList = scanRelElement.getChildNodes();
            Integer scanRel = Integer.parseInt(
                              ((Node)scanRelList.item(0)).getNodeValue());
            
            ProductComponentInfo [] pcis = 
               ecb.getServiceConnection3().getService().retrieveProductComponents(
               ecb.getServiceConnection3().getServiceInstanceRef());
            boolean toInstall = true;
            for(int k=0; k<pcis.length; k++) {
               ProductComponentInfo pci = pcis[k];
            ...
            if(toInstall) {
               ...
               Document contentDoc = 
                  getDocument(ecb.get_option("targetDirectory") 
   ...
   private static String getbundleProductStr() throws Exception {
      Document doc = getDocument(ecb.get_option("targetDirectory") 
                               + "/" + BUNDLE_METADATA);
      Element nameElement = (Element)doc.getDocumentElement().getElementsByTagName(
                             "name").item(0);
      NodeList nameList = nameElement.getChildNodes();                   
      String name = ((Node)nameList.item(0)).getNodeValue();
      
      Element versionElement = (Element)doc.getDocumentElement().getElementsByTagName(
                               "version").item(0);
      NodeList versionList = versionElement.getChildNodes();
      String version = ((Node)versionList.item(0)).getNodeValue();                   
      
      Element localeElement = (Element)doc.getDocumentElement().getElementsByTagName(
                               "locale").item(0);
      NodeList localeList = localeElement.getChildNodes();                   
      String locale = ((Node)localeList.item(0)).getNodeValue();
   ...
   private static boolean verifySignatures(ArrayList fileList) throws Exception {
      ...
      for(int i=0; i < listOfPackage.getLength() ; i++) {
         ...
         if(packageNode.getNodeType() == Node.ELEMENT_NODE) {
            Element packageElement = (Element)packageNode;                                          
            NodeList packageName = packageElement.getElementsByTagName(
                                                 "packageContentsUrl");
            if(packageName.getLength() > 0) {
               Element packageNameElement = (Element)packageName.item(0);
               NodeList packageNameList = packageNameElement.getChildNodes();                   
               String oName = ((Node)packageNameList.item(0)).getNodeValue();
               boolean found = foundFile(fileList,
                                         oName.substring(oName.lastIndexOf("/")));
               if(found) {
                  System.out.println("Package    " + 
                                     oName.substring(oName.lastIndexOf("/")+1)
                                     +" found");                  
                  unzipMember(ecb.get_option("targetDirectory")+"/"+
                            oName.substring(oName.lastIndexOf("/")+1));
               }
               else {
            ...
            packageName = packageElement.getElementsByTagName(
                                         "signature");
            if(packageName.getLength() > 0) {
         ...
         if(!entry.isDirectory()) {
            ...
            copyInputStream(
               ...
               new BufferedOutputStream(
                  new FileOutputStream(ecb.get_option("targetDirectory")+"/"+
                  entry.getName().substring(entry.getName().lastIndexOf("/")))));
   ...
   private static void unzipMember(String fileName) throws Exception {  	  
      ...
      while(entries.hasMoreElements()) {
         ...
         if(!entry.isDirectory()) {
            boolean success = 
               (new File(ecb.get_option("targetDirectory") + "/" 
               + entry.getName().substring(0,entry.getName().indexOf("/")))).mkdirs();              
            copyInputStream(
               zipFile.getInputStream(entry),
               new BufferedOutputStream(
                  new FileOutputStream(ecb.get_option("targetDirectory")+"/"+
   ...
   private static void connectV25(String [] args, String cookieString) throws Exception {      
      ecb = ExtendedAppUtil.initialize("InstallHostPatch"
   ...
   private static void putFiles(String localFilePath,String pName) 
      ...
      String httpUrl = serviceUrl+"/tmp/"+PATCH_DIR+"/"+pName+"/"+localFilePath.substring(
