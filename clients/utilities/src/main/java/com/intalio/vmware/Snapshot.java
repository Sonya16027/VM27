...
public class Snapshot {
	...
	public void createSnapShot(String vmName, String ssName) {
		try {
			 ...
			 ********************************
			 */
			ManagedObjectReference cssTask = connection.getVimPort().createSnapshot_Task(
	...
	public void removeSnapShot(ManagedObjectReference vmss,
		...
		try {
			ManagedObjectReference rssTask = connection.getVimPort().removeSnapshot_Task(vmss,
					removeChildren);
	...
	public Object[] waitForValues(ManagedObjectReference objmor,
		...
		ManagedObjectReference filterSpecRef = connection.getVimPort().createFilter(
				propertyCollector, spec, true);
		...
		while (!reached) {
			updateset = connection.getVimPort().waitForUpdates(propertyCollector, version);
