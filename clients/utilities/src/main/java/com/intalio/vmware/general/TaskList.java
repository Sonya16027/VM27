...
/**
 *<pre>
 * his sample dipslays a list of tasks performed on some managed 
 * bject. The sample display the tasks in the following format:
 * 
 * peration:
 * ame:
 * ype:
 ...
 * rror:
 * 
 * f there is no task running when this sample is used, then it
 * isplays a messsage.
 * 
 * b&gt;Command:&lt;/b&gt;
 * un.bat com.intalio.vmware.general.TaskList --url [webserviceurl]
...
public class TaskList {
	...
	public static void main(String[] args) throws Exception {
		...
		ManagedObjectReference taskManagerRef = cb.getConnection()
		...
		ObjectContent[] oContents = cb.getConnection().getService()
				.retrieveProperties(propColl, pfs);
		if (oContents != null) {
	...
	protected PropertyFilterSpec[] createPFSForRecentTasks(
		...
		pSpec.setType("Task");
		pSpec.setPathSet(new String[] { "info.entity", "info.entityName",
				"info.name", "info.state", "info.cancelled", "info.error" });
			...
			if (dps != null) {
				...
				for (int dpi = 0; dpi < dps.length; ++dpi) {
					...
					if ("info.entity".equals(dp.getName())) {
						...
						LocalizedMethodFault mf = (LocalizedMethodFault) dp
								.getVal();
						if (mf != null) {
