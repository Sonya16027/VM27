...
import com.intalio.vmware.performance.widgets.LineChart;
...
import com.vmware.vim.PerfEntityMetricCSV;
import com.vmware.vim.PerfInterval;
import com.vmware.vim.PerfMetricId;
import com.vmware.vim.PerfMetricSeriesCSV;
 ...
 * <li>counter Required. Full name of counter in dotted notation
 * (group.counter.rollup). For example, "cpu.usage.none"</li>
 * </ul>
 * <p>
 * <b>Command Line:</b>
 * <p>
 * run.bat com.intalio.vmware.performance.VIUsage --url [webserviceurl]
...
public class VIUsage implements ActionListener {
	...
	private static OptionSpec[] constructOptions() {
		...
		useroptions[1] = new OptionSpec("counter", "String", 1,
				"Type of Counter [group.counter.rollup]"
						+ " e.g. cpu.usage.none", null);
		return useroptions;
	}

	public static void main(String[] args) throws Exception {
		VIUsage viUsage = new VIUsage();
		cb = AppUtil.initialize("VIUsage", VIUsage.constructOptions(), args);
		cb.connect();
	...
	private void createMainPanel() {
		...
		chartLabel.setVerticalTextPosition(SwingConstants.CENTER);
		chartLabel.setHorizontalTextPosition(SwingConstants.CENTER);
		...
		selectPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory
				.createTitledBorder("Interval"), BorderFactory
				.createEmptyBorder(5, 5, 5, 5)));
		displayPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory
				.createTitledBorder("Metric Name"), BorderFactory
				.createEmptyBorder(5, 5, 5, 5)));
	...
	private void initChart() {
		...
		chart = new LineChart(tickInterval, period * 1000, format, format,
				movingAvg, true);
	...
	private void updateChart() {
		...
		if (counterMap.containsKey(key)) {
			...
			String instance = "";
			metricIds[0] = new PerfMetricId(null, null, counterInfo.getKey(),
					instance);
		} else {
		...
		try {
			metrics = service.queryPerf(perfMgr,
		...
		displayPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory
				.createTitledBorder(statName), BorderFactory.createEmptyBorder(
				5, 5, 5, 5)));
	}
}