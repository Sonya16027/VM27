 ...
 * 
 * b&gt;Command line&lt;/b&gt;
 * un.bat com.intalio.vmware.ciminfo.VIWrapper --url [webserviceurl]
...
public class VIWrapper {
	...
	protected void doOperation() throws Exception {
		...
		if (apiType.equalsIgnoreCase("HostAgent")
			...
			boolean printNulls = appUtil.get_option(OPT_PRINTNULLS)
					.equalsIgnoreCase("true");
			boolean printKeys = appUtil.get_option(OPT_PRINTONLYKEYS)
					.equalsIgnoreCase("true");
			boolean ignoreCert = appUtil.get_option(OPT_IGNORECERT)
					.equalsIgnoreCase("true");
